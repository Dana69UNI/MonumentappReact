...
import { useNavigate } from 'react-router-dom'; // <--- 1. Importem això
      ...
      {caselles.map((id) => {
        ...
        return (
          <div 
            ...
            // <--- 3. AFEGIM EL CLICK (Porta a la fitxa detall)
            onClick={() => navigate(`/cercar/${id}`)} 
          >
            {isVisited ? (
              <img src={TreeIcon} alt={`Arbre ${id}`} className="icona-arbre-graella" />
            ) : (
